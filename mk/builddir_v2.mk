...
ifneq ($(strip ${STATICLIB_NAME}), )
	...
	LIB_TARGET   = $(PRJ_LIBDIR)/$(PLATFORM)/lib$(STATICLIB_NAME)$(STATICLIBSUFFIXNAME)
	LIB_TARGET_DBG = $(PRJ_LIBDIR)/$(PLATFORM)/lib$(STATICLIB_NAME)_$(DEBUG_SUFFIX)$(STATICLIBSUFFIXNAME)
endif

ifneq ($(strip ${SHARDLIB_NAME}), )
	COMPILE_PATH = ${PRJ_COMPILEDIR}/${PLATFORM}/${SHARDLIB_NAME}/
	SHARDLIB_TARGET   = $(PRJ_LIBDIR)/$(PLATFORM)/lib$(SHARDLIB_NAME)$(SHARDLIBSUFFIXNAME)
	SHARDLIB_TARGET_DBG = $(PRJ_LIBDIR)/$(PLATFORM)/lib$(SHARDLIB_NAME)_$(DEBUG_SUFFIX)$(SHARDLIBSUFFIXNAME)
endif

ifneq ($(strip ${APP_NAME}), )
	COMPILE_PATH = ${PRJ_COMPILEDIR}/${PLATFORM}/${APP_NAME}/
	APP_TARGET = $(PRJ_BINDIR)/$(PLATFORM)/$(APP_NAME)
	APP_TARGET_DBG = $(PRJ_BINDIR)/$(PLATFORM)/$(APP_NAME)_$(DEBUG_SUFFIX)
endif
