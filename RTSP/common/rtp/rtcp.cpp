#include "rtcp.h"

static unsigned  char firstheartbeatData[] = { 0x80 ,0xC9 ,0x00 ,0x01 ,0x80 ,0x00 ,0x00 ,0x29 ,0x81 ,0xCA ,0x00 ,0x04
			,0x80 ,0x00 ,0x00 ,0x29 ,0x01 ,0x09 ,0x62 ,0x6C  ,0x75 ,0x65 ,0x73 ,0x74 ,0x6F ,0x72 ,0x6D, 0x00 };

static unsigned  char normaltheartbeatData[] = { 0x81, 0xc9, 0x00, 0x07, 0x27, 0xf7, 0x0a, 0x8d, 0x04, 0xfe, 0x32, 0x09
			,0x00, 0xff, 0xff, 0xff, 0x00, 0x01, 0x99, 0x6a, 0x00, 0x00, 0x03, 0x90, 0x0a, 0xda, 0x00, 0x00
			,0x00, 0x00, 0x23, 0x91, 0x81, 0xca, 0x00, 0x04, 0x27, 0xf7, 0x0a, 0x8d, 0x01, 0x07, 0x52, 0x41
			,0x4e, 0x44, 0x4f, 0x4e, 0x47, 0x00, 0x00, 0x00 };

std::string firstRtpOverTcpRTCPHeartBeat()
{
	return std::string((const char*)firstheartbeatData, sizeof(firstheartbeatData));
}
std::string normalRtpOverTcpRTCPHeartBeat()
{
	return std::string((const char*)normaltheartbeatData, sizeof(normaltheartbeatData));
}