...
{
	static bool networkinitflag = false;
...
std::string Host::getSystemDisk()
{
	std::string systemdisk;
#ifdef WIN32
	char systempath[513];
	GetSystemDirectory(systempath, 512);
	size_t i = String::indexOfByCase(systempath, "\\Windows\\system32");
	if (i == -1)
	{
		i = String::indexOfByCase(systempath, "/Windows/system32");
	}
	if (i != -1)
	{
		systemdisk = std::string(systempath, i);
	}
#else
	systemdisk = "/";
#endif
...
bool Host::getNowUsedPortMap(std::set<uint16_t> &portmap, SocketType type)
{
	...
	while ((tmp = fgets(buffer, 255, fd)) != NULL)
	{
		...
		while (*pftmpend == ':' || (*pftmpend >= '0' && *pftmpend <= '9'))
			pftmpend++;
...
{
	std::string sysdisk = getSystemDisk() + "\\";
	...
	for (unsigned i = 0; i < len; i += 4)
	{
		...
		info.IsSystemDisk = String::strcasecmp(pathname, sysdisk) == 0;
...

		{
			...
			if (GetVolumeInformation(pathname.c_str(), volumeNmaeBuff, MAX_PATH, &volumeSerialNumber, &maximumComponentLength, &fileSystemFlag, fileSystemName, MAX_PATH))
			{
				...
				if (String::iequals(fileSystemName, "ntfs"))
					info.FormatType = DiskInfo::FormatType_NTFS;
				else if (String::strncasecmp(fileSystemName, "fat", 3) == 0)
					info.FormatType = DiskInfo::FormatType_FAT;
			}
			else
			{
				//int b = 0;
...
std::map<std::string, Host::NetworkInfo> Host::getNetworkInfos()
{
	...
	while (pIterater)
	{
		info.adapterName = std::string("\\Device\\NPF_") + pIterater->AdapterName;
		info.description = pIterater->Description;
		{
			char buffer[255];
			snprintf_x(buffer, 255, "%02X:%02X:%02X:%02X:%02X:%02X", pIterater->Address[0] & 0xff, pIterater->Address[1] & 0xff, pIterater->Address[2] & 0xff, pIterater->Address[3] & 0xff, pIterater->Address[4] & 0xff, pIterater->Address[5] & 0xff);
			info.mac = buffer;
		}
