...
struct ShareMEMBuffer::ShareMEMBufferInternal : public Thread
{
	...
	ShareMEMBufferInternal(const std::string &shareName, int createBlock, int maxBlock, int rblockSize, int maxrblock, int memMaxSize, bool _create, void *startAddr, const ReadMEMCallback &_callback)
		: Thread("ShareMEMBufferInternal"), createWriteList(NULL), createWritePos(NULL), createReadPos(NULL), createMaxSize(0), createBlockSize(0), readWriteList(NULL), readWritePos(NULL), readReadPos(NULL), readMaxSize(0), readBlockSize(0), create(_create), callback(_callback)
	{
...
{
	if (internal == NULL || internal->sharemem == NULL)
		return NULL;
