...
struct String::StringInternal
{
	...
	void alloc(size_t size)
	{
		...
		if (size > 0)
		{
			newbuffer = make_shared<StringBufer>(mempool, (uint32_t)size + 100);
		}
	...
	void setval(const char* ptr, size_t size)
	{
		...
		if (buffer == NULL || size+1 > buffer->bufferSize)
		{
			alloc(size+1);
		}
		...
		buffer->buffer[buffer->dataLength] = 0;
	}
	...
	void append(const char* ptr, size_t size)
	{
		...
		if (buffer == NULL || size + buffer->dataLength + 1 > buffer->bufferSize)
		{
			shared_ptr<StringBufer> nowbuffer = buffer;

			alloc(buffer == NULL ? size + 1 : size + buffer->dataLength + 1);
		...
		if (buffer == NULL || buffer->buffer == NULL || buffer->dataLength + size + 1 > buffer->bufferSize) return;
