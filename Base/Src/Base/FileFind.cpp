...
static long base_findfirst(const std::string& fileName, base_finddata_t* finddata)
{
	if(finddata == NULL)
	{
		return 0;
...
static long base_findnext(long handle, base_finddata_t* finddata)
{
...
static void base_findclose(long handle)
{
	if(handle <= 0)
	{
		return;
...
#else
#ifdef _WIN64
#define base_findfirst	_findfirst64
#define base_finddata_t	__finddata64_t
#define base_findnext	_findnext64
#define base_findclose	_findclose
#else
#define base_findfirst	_findfirst
#define base_finddata_t	_finddata_t
#define base_findnext	_findnext
#define base_findclose	_findclose
#endif
#endif
...
FileFind::~FileFind()
{
	...
	{
		base_findclose(internal->handle);
		internal->handle = -1;
	}
	delete internal;
}
...
{
	base_finddata_t finddata = { 0 };

	while (true)
	{
		if (internal->handle == (size_t)-1)
		{
			internal->handle = base_findfirst(internal->searchfile.c_str(), &finddata);
			if (internal->handle == (size_t)-1)
			{
		...
		{
			long ret = base_findnext(internal->handle, &finddata);
			if (ret < 0)
			{
				return shared_ptr<FileFind::FileFindInfo>();
			}
...
FileFind::FileFindInfo::FileFindInfo(const std::string& path, void* findinfo)
{
	...
	internal->fileInfo = *(base_finddata_t*)findinfo;
	internal->path = path;
}
