...
struct ThreadPool::ThreadPoolInternal
{
	...
	ThreadPoolInternal(uint32_t threadnum, Thread::Priority pri)
	{
	...
	//	SetThreadpoolThreadMinimum(tp_pool, 1);
		...
		if (pri == Thread::priorTop)
		{
			SetThreadpoolCallbackPriority(&tp_env, TP_CALLBACK_PRIORITY_HIGH);
		}
		else if (pri == Thread::priorBottom)
		{
			SetThreadpoolCallbackPriority(&tp_env, TP_CALLBACK_PRIORITY_LOW);
		}
		else
		{
			SetThreadpoolCallbackPriority(&tp_env, TP_CALLBACK_PRIORITY_NORMAL);
		}
...
{
public:
	ThreadPoolThread(Thread::Priority pri) :Thread("ThreadPoolThread", pri),prevusedtime(Time::getCurrentMilliSecond()), idelflag(false)
	{
		createThread();
	}
	~ThreadPoolThread()
...
public:
	...
	bool dispatch(const ThreadPool::Proc& func, void* param)
	{
		...
		shared_ptr< ThreadPoolThread> canusedthread;
		{
			...
			{
				canusedthread = make_shared<ThreadPoolThread>(threadpri);
...
ThreadPool::ThreadPool(uint32_t threadnum, Thread::Priority pri)
{
	internal = new ThreadPoolInternal(threadnum,pri);
}
