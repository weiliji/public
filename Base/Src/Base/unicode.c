...
{ 
	...
	int len= wcstombs(tgt, src, (int)tgtBytes);
	if (len < 0) return 0;
	return len;
#endif
}
...
int unicode_from_mbcs(IN const char* src, IN size_t srcLen, OUT wchar_t* tgt, IN size_t tgtBytes)
{
	...
	int len= mbstowcs(tgt, src, (int)tgtBytes);
	if (len < 0) return 0;
...
int unicode_to_gb18030(IN const wchar_t* src, IN size_t srcLen, OUT char* tgt, IN size_t tgtBytes)
{
	...
	int len = gb2312_wctomb(src, (unsigned char*)tgt, (int)tgtBytes);
	if (len < 0) return 0;
	return len;
#endif
} 
...
int unicode_from_gb18030(IN const char* src, IN size_t srcLen, OUT wchar_t* tgt, IN size_t tgtBytes)
{
	...
	int len = gb2312_mbtowc((unsigned char*)src, tgt, (int)tgtBytes);
	if (len < 0) return 0;
	return len;
#endif
}
