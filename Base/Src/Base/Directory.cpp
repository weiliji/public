...
static uint64_t directoryFileSize(const std::string &filename)
{
	uint64_t fileSize = 0;
	...
	do
	{
		Directory::Dirent direct;
		if (!directory.read(direct))
		{
			break;
		}

		if (direct.Type == Directory::Dirent::DirentType_Dir)
		{
			fileSize += directoryFileSize(direct.CompletePath);
		}
		else if (direct.Type == Directory::Dirent::DirentType_File)
		{
...
bool Directory::read(Dirent &dir)
{
	while (true)
	{
		...
		dir.ModifyTime = finfo.timeWrite;
...
{
	return directoryFileSize(internal->path);
}
