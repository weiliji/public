...
public:
	StaticMemPoolInternal(char* addr,int size,IMutexInterface* lock,bool create, bool _sharedheader, uint32_t chunksize):listHeader(NULL),nodeHeader(NULL),memChunkSize(chunksize)
	{
		sharedheader = _sharedheader;
		bufferUsedSize = 0;
		...
		{
			listHeader = new NodeIndexList[nodeIndexSize + 1];
			nodeHeader = new ChunkNode[chunkSize];

			realDataBufferAddr = bufferStartAddr;
		}
		if(!create)
		{
			return;
	...
	~StaticMemPoolInternal()
	{
		if (!sharedheader)
		{
			SAFE_DELETEARRAY(listHeader);
			SAFE_DELETEARRAY(nodeHeader);
		}
	//	poolTimer = NULL;
	}
...
StaticMemPool::StaticMemPool(char* bufferStartAddr,int bufferSize,IMutexInterface* locker, bool create, bool sharedheader, uint32_t chunksize)
{
	if (chunksize == 0) chunksize = DefaultMemChunkSize;

	internal = new StaticMemPoolInternal(bufferStartAddr, bufferSize, locker, create, sharedheader,chunksize);
}
