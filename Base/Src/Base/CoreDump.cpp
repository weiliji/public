...
void findAndCheckCore()
{
	...
	while (true)
	{
		...
		if (File::access(filename.c_str(), File::accessExist))
		{
			...
			SystemCall(buffer);
#ifdef WIN32
			const char *fileprec = ".dmp";
#else
			const char *fileprec = ".core";
#endif
			char dumpfilename[256] = {0};
			snprintf_x(dumpfilename, 255, "%s/%s_%04d%02d%02d%02d%02d%02d%s", fileprec, getRecordPath(dumpSavePath).c_str(), File::getExcutableFileName().c_str(), creatFileTime.year, creatFileTime.month, creatFileTime.day, creatFileTime.hour, creatFileTime.minute, creatFileTime.second);
...
struct CoreDumpFileInfo
{
	...
	bool operator<(const CoreDumpFileInfo &info) const
	{
