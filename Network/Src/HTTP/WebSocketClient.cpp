...
struct WebSocketClientManager :public HTTPCommunicationHandler, public enable_shared_from_this<WebSocketClientManager>
{
	...
	void socketconnectcallback(const weak_ptr<Socket>& sock, bool status, const std::string& err)
	{
		...
		commusession = make_shared<Communication>(false, socket, shared_from_this(), useragent);
		commusession->recvHeader = make_shared<Header>();
	...
	virtual bool onRecvHeaderOk(Communication* commu) { return true; }
...
WebSocketClient::WebSocketClient(const shared_ptr<IOWorker>& worker, const std::string& useragent)
{
	...
	internal->manager->sendheader = make_shared<Header>();
...
shared_ptr<Header> WebSocketClient::header()
{
	return internal->manager->sendheader;
}
