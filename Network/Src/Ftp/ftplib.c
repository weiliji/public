...
int FtpOptions(int opt, long val, netbuf *nControl)
{
	...
	case FTPLIB_CALLBACK:
		nControl->idlecb = (FtpCallback)(size_t)val;
		rv = 1;
		break;
	...
	case FTPLIB_CALLBACKARG:
		rv = 1;
		nControl->idlearg = (void *)(size_t)val;
		break;
...
static int FtpSendCmd(const char *cmd, netbuf *nControl)
{
	...
	if ((strlen(cmd) + 3) > sizeof(buf))
		return 0;
	sprintf(buf, "%s\r\n", cmd);
	if (net_write(nControl->handle, buf, (int)strlen(buf)) <= 0)
	{
		if (ftplib_debug)
			perror("write");
		return 0;
...
static int FtpAcceptConnection(netbuf *nData, netbuf *nControl)
{
	...
	i = (int)nControl->handle;
	if (i < (int)nData->handle)
		i = (int)nData->handle;
