...
{
    type = media_type;
    subtype = String::tolower(media_subtype);
}
...
mime::content_type_t& mime::content_type_t::operator=(const mime::content_type_t& cont_type)
{
    ...
    {
        type = cont_type.type;
        subtype = String::tolower(cont_type.subtype);
...
void mime::content_type(media_type_t media_type, const string& media_subtype, const string& charset)
{
    ...
    c.charset = String::tolower(charset);
    content_type(c);
}
...
void mime::parse_header_line(const string& header_line)
{
    ...
    if (String::iequals(header_name, CONTENT_TYPE_HEADER))
    {
        ...
        _content_type.subtype = String::tolower(media_subtype);
        ...
        if (charset_it != attributes.end())
            _content_type.charset = String::tolower(charset_it->second);
    ...
    else if (String::iequals(header_name, CONTENT_TRANSFER_ENCODING_HEADER))
    {
        attributes_t attributes;
        parse_content_transfer_encoding(header_value, _encoding, attributes);
    }
    else if (String::iequals(header_name, CONTENT_DISPOSITION_HEADER))
    {
        attributes_t attributes;
...
void mime::parse_content_transfer_encoding(const string& transfer_encoding_hdr, content_transfer_encoding_t& encoding, attributes_t& attributes) const
{
    ...
    if (String::iequals(header_value, CONTENT_TRANSFER_ENCODING_BASE64))
        encoding = content_transfer_encoding_t::BASE_64;
    else if (String::iequals(header_value, CONTENT_TRANSFER_ENCODING_QUOTED_PRINTABLE))
        encoding = content_transfer_encoding_t::QUOTED_PRINTABLE;
    else if (String::iequals(header_value, CONTENT_TRANSFER_ENCODING_BIT7))
        encoding = content_transfer_encoding_t::BIT_7;
    else if (String::iequals(header_value, CONTENT_TRANSFER_ENCODING_BIT8))
        encoding = content_transfer_encoding_t::BIT_8;
    else if (String::iequals(header_value, CONTENT_TRANSFER_ENCODING_BINARY))
...
void mime::parse_content_disposition(const string& content_disp_hdr, content_disposition_t& disposition, attributes_t& attributes) const
{
    ...
    if (String::iequals(header_value, CONTENT_DISPOSITION_ATTACHMENT))
        disposition = content_disposition_t::ATTACHMENT;
    else if (String::iequals(header_value, CONTENT_DISPOSITION_INLINE))
...
string mime::make_boundary() const
{
    ...
    std::uniform_int_distribution<> index_dist(0, (int)codec::HEX_DIGITS.size() - 1);
    for (int i = 0; i < 10; i++)
...
mime::media_type_t mime::mime_type_as_enum(const string& media_type_val) const
{
    ...
    if (String::iequals(media_type_val, "text"))
        media_type = media_type_t::TEXT;
    else if (String::iequals(media_type_val, "image"))
        media_type = media_type_t::IMAGE;
    else if (String::iequals(media_type_val, "audio"))
        media_type = media_type_t::AUDIO;
    else if (String::iequals(media_type_val, "video"))
        media_type = media_type_t::VIDEO;
    else if (String::iequals(media_type_val, "application"))
        media_type = media_type_t::APPLICATION;
    else if (String::iequals(media_type_val, "multipart"))
        media_type = media_type_t::MULTIPART;
    else if (String::iequals(media_type_val, "message"))
