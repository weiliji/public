...
{	
	_date_time = _date_time.getCurrentTime();
}
...
Time message::date_time() const
{
    return _date_time;
}


void message::date_time(const Time& mail_dt)
{
    _date_time = mail_dt;
}
...
void message::parse_header_line(const string& header_line)
{
    ...
    if (String::iequals(header_name, FROM_HEADER))
    {
        _from = parse_address_list(header_value);
        if (_from.addresses.empty())
            throw message_error("Empty author header.");
    }
    else if (String::iequals(header_name, SENDER_HEADER))
    {
        mailboxes mbx = parse_address_list(header_value);
        if (!mbx.addresses.empty())
            _sender = mbx.addresses[0];
    }
    else if (String::iequals(header_name, REPLY_TO_HEADER))
    {
        mailboxes mbx = parse_address_list(header_value);
        if (!mbx.addresses.empty())
            _reply_address = mbx.addresses[0];
    }
    else if (String::iequals(header_name, TO_HEADER))
    {
        _recipients = parse_address_list(header_value);
    }
    else if (String::iequals(header_name, CC_HEADER))
    {
        _cc_recipients = parse_address_list(header_value);
    }
    else if (String::iequals(header_name, SUBJECT_HEADER))
        _subject = parse_subject(header_value);
    else if (String::iequals(header_name, DATE_HEADER))
        _date_time = parse_date(header_value);
    else if (String::iequals(header_name, MIME_VERSION_HEADER))
        _version = header_value;
}
...
string message::format_address_list(const mailboxes& mailbox_list) const
{
   const RegEx ATEXT_REGEX{R"([a-zA-Z0-9\!#\$%&'\*\+\-\./=\?\^\_`\{\|\}\~]*)"};
    ...
    {
        if (!RegEx::regex_match(mg->name, ATEXT_REGEX))
...
string message::format_address(const string& name, const string& address) const
{
    ...
    {
        if (RegEx::regex_match(name, RegEx(R"([A-Za-z0-9\ \t]*)")))
            name_formatted = name;
        else if (RegEx::regex_match(name, QTEXT_REGEX))
    ...
    if (!address.empty())
    {
        if (RegEx::regex_match(address, DTEXT_REGEX))
    ...
    for (auto ch = address_list.begin(); ch != address_list.end(); ch++)
    {
        switch (state)
        {
            ...
            case state_t::NAMEADDRGRP:
            {
                ...
                {
                    cur_address.name = String::trim_copy(token);
